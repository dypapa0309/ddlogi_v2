  ...
  const itemsCost = items;
    ...
    function ensureAiAssistantMounted() {
      ...
      const itemAliasEntries = [
        ...
        ["청소기", ["청소기"]],
        ["정수기(이동만)", ["정수기"]],
        ["TV/모니터", ["tv", "티비", "티브이", "인치티비", "인치 tv", "모니터"]],
        ["의자", ["의자", "식탁의자"]],
        ["행거", ["행거"]],
        ["협탁/사이드테이블(소형)", ["협탁", "사이드테이블"]],
        ["화장대(소형)", ["화장대"]],
        ["책상/테이블(일반)", ["책상", "테이블", "원형테이블", "식탁"]],
        ["서랍장(3~5단)", ["서랍장"]],
        ["책장(일반)", ["책장", "선반"]],
      ...
      // ==== BEGIN MODIFICATION: expose parsing helpers early ====
      // Some downstream functions (e.g. serverless fallbacks or modal helpers)
      // may attempt to reference parseMoveDetails, parseCleanDetails or
      // extractItems from the global scope. When running in a file:// context
      // those helpers live inside this IIFE and are therefore not visible on
      // window until the end of the script. In practice, async tasks such as
      // OCR progress callbacks may fire before the end-of-file assignment
      // executes, leading to errors like "parseMoveDetails is not defined" or
      // "extractItems is not defined". To avoid this, expose the helpers
      // immediately after their definitions. We guard each assignment so we
      // don't clobber any existing global definitions.
      try {
        if (typeof parseMoveDetails === 'function' && !window.parseMoveDetails) {
          window.parseMoveDetails = parseMoveDetails;
        }
        if (typeof parseCleanDetails === 'function' && !window.parseCleanDetails) {
          window.parseCleanDetails = parseCleanDetails;
        }
        if (typeof extractItems === 'function' && !window.extractItems) {
          window.extractItems = extractItems;
        }
        // Expose other low‑level parsing helpers that may be referenced
        // by downstream modules.  parseDateValue and parseTimeValue are used
        // to derive dates and time slots from arbitrary text when building
        // fallback move details.
        if (typeof parseDateValue === 'function' && !window.parseDateValue) {
          window.parseDateValue = parseDateValue;
        }
        if (typeof parseTimeValue === 'function' && !window.parseTimeValue) {
          window.parseTimeValue = parseTimeValue;
        }
        // Expose additional helpers used by offline fallback and other modules.
        if (typeof mergeIntoState === 'function' && !window.mergeIntoState) {
          window.mergeIntoState = mergeIntoState;
        }
        if (typeof applyStateToDom === 'function' && !window.applyStateToDom) {
          window.applyStateToDom = applyStateToDom;
        }
        if (typeof refreshTimeSlotAvailability === 'function' && !window.refreshTimeSlotAvailability) {
          window.refreshTimeSlotAvailability = refreshTimeSlotAvailability;
        }
        if (typeof tryAutoDistanceCalc === 'function' && !window.tryAutoDistanceCalc) {
          window.tryAutoDistanceCalc = tryAutoDistanceCalc;
        }
        if (typeof renderAll === 'function' && !window.renderAll) {
          window.renderAll = renderAll;
        }
        if (typeof getRecommendedStepIndex === 'function' && !window.getRecommendedStepIndex) {
          window.getRecommendedStepIndex = getRecommendedStepIndex;
        }
      } catch (ex) {
        console.warn('Failed to assign parsing helpers globally (early):', ex);
      }
      // ==== END MODIFICATION ====
      ...
      async function callAiAssistAPI({ message, targetService }) {
        // Disable calls to the remote AI assistant when running offline.  When
        // this function is invoked we immediately throw to signal that no
        // server response is available.  The caller will catch this and
        // continue with locally parsed information only.  This avoids
        // uncaught promise rejections and opaque error messages when the
        // OPENAI_API_KEY environment variable is missing.
        throw new Error("AI assist disabled in offline mode");
      }
...
${snapshotSummary}`);
    ...
    function normalizeOcrLine(line) {
      return normalizeWhitespace(String(line || "")
        ...
        .replace(/[•·]/g, " ")
        .replace(/[‐‑‒–—]/g, "-")
        .replace(/(?:soonpfj\w*|LA\s+DERIVE|만능\s*레버|파워\s*gl|광고|찾으셨나요)/gi, " ")
        .replace(/\d{1,2}:\d{2}/g, " ")
    ...
    async function callCaptureVisionAPI(files) {
      // In offline mode we can't reach the serverless Vision API.  Instead of
      // attempting a network call that will fail and surface confusing
      // exceptions (for example, "extractItems is not defined" or missing
      // environment variables), immediately throw an error.  The surrounding
      // logic in handleCaptureUpload will catch this and fall back to
      // client‑side OCR via Tesseract.  This preserves the original flow
      // where OCR is used as a secondary source when remote analysis is
      // unavailable.
      throw new Error("vision API disabled in offline mode");
    }

    async function callPhotoConsultAPI(files) {
      /*
       * When the application is served online (http or https) call the
       * serverless photo consultation API.  This replicates the original
       * behaviour from app.original.js.  When offline (file protocol) we
       * proceed with the existing client‑side OCR fallback below.
       */
      {
        const proto = (typeof window !== 'undefined' && window.location)
          ? window.location.protocol
          : '';
        if (proto && !/^file/i.test(proto)) {
          // Prepare images and call remote API
          const images = [];
          for (let i = 0; i < files.length; i += 1) {
            updatePhotoConsultProgress(
              `사진 ${i + 1}/${files.length} 준비 중`,
              10 + Math.round((i / Math.max(1, files.length)) * 18),
              "상담용으로 사진 크기를 줄이고 큰 짐이 잘 보이게 준비하고 있어요."
            );
      ...
      // Otherwise (offline) fall back to a purely client‑side implementation using OCR.
      const ocrText = await extractTextFromCaptureFiles(files);
      // Default values in case OCR fails to read anything meaningful
      let parsed = { items: {} };
      try {
        parsed = parseMoveDetails(ocrText);
      } catch (err) {
        // If parsing fails for any reason, keep an empty parsed object
        parsed = { items: {} };
      }
      const items = Object.entries(parsed.items || {});
      // Build lists for visible (large) items and small items.  Treat items
      // with counts >= 1 as visible items, and anything labelled 소형짐 as
      // small.  Remaining items are also considered visible.
      const visibleItems = [];
      const smallItems = [];
      items.forEach(([name, count]) => {
        if (name === "소형짐") smallItems.push(`${name} ${count}개`);
        else visibleItems.push(`${name} ${count}개`);
      });
      // Estimate load level based on total item count
      const totalCount = items.reduce((sum, [, qty]) => sum + (Number(qty) || 0), 0);
      let loadEstimate = "추정이 어려워요.";
      if (totalCount >= 7) loadEstimate = "대형 짐량";
      else if (totalCount >= 4) loadEstimate = "중간 짐량";
      else if (totalCount >= 1) loadEstimate = "소형 짐량";
      // Suggest follow‑up questions if no items detected
      const followUpQuestions = items.length
        ? []
        : ["추가로 보내주신 짐이 있다면 알려주세요."];
      // Build customer message summarising recognised items
      const customerMessage = items.length
        ? `사진을 기준으로 ${items.map(([n, c]) => `${n} ${c}개`).join(", ")}가 확인됩니다. 추가 짐이나 요청사항이 있으면 알려주세요.`
        : "사진에서 확실한 짐을 인식하지 못했습니다. 문자로 상담을 진행해주세요.";
      // Build counselor note summarising visible and small items
      const counselorNote = items.length
        ? `보이는 큰 짐: ${visibleItems.join(", ") || "없음"}\n잔짐 / 생활짐: ${smallItems.join(", ") || "없음"}`
        : "사진에서 확실한 짐을 인식하지 못했습니다.";
      return {
        ok: true,
        summary: customerMessage,
        extractedText: ocrText || "",
        confidence: items.length ? 0.8 : 0.35,
        visibleItems,
        smallItems,
        loadEstimate,
        followUpQuestions,
        customerMessage,
        counselorNote,
        userGuide: "문자앱 또는 공유창에서 사진을 마지막에 직접 첨부해주세요.",
        title: items.length ? "사진 기준 상담 요약이 준비됐어요." : "사진을 이해하지 못했어요",
        shouldRecommendSms: items.length === 0,
        unreadableReason: items.length === 0 ? "사진에서 짐을 인식하지 못했습니다" : undefined,
      };
    }
    ...
    function normalizeParsedShapeCompat(candidate) {
      if (!candidate || typeof candidate !== "object") return {};
      const parsed = { ...candidate };
      if (parsed.items && typeof parsed.items === "object") {
        parsed.items = Object.fromEntries(Object.entries(parsed.items).filter(([, v]) => Number(v) > 0));
    ...
    function normalizeParsedShapeSafe(candidate) {
      try {
        return normalizeParsedShapeCompat(
          typeof normalizeParsedShape === "function" ? normalizeParsedShape(candidate) : candidate
        );
      } catch (_) {
        return normalizeParsedShapeCompat(candidate);
      }
    }
    ...
    function findAddressCandidates(raw) {
      ...
      const candidates = [];
      const region = '(?:서울|경기|인천|부천|광주|대전|대구|부산|울산|세종|제주|강원|충북|충남|전북|전남|경북|경남)';
      const push = (value) => {
        const cleaned = normalizeWhitespace(String(value || '')
          .replace(/^(출발지|도착지|경유지|주소)\s*(는|은|이|가)?\s*/, '')
          .replace(/^(어디인가요\?|어디예요\?|어디인가요|어디예요)\s*/, '')
          .replace(/(출발지에|도착지에)\s*엘리베이터.*$/, '')
          .replace(/(있나요\?|없나요\?).*$/, '')
          .replace(/[,.]$/g, '')
        );
        if (!cleaned) return;
        if (!new RegExp(`${region}\\s`).test(cleaned) && !/(?:시|군|구)\s+.+(?:동|읍|면)/.test(cleaned)) return;
        if (!candidates.includes(cleaned)) candidates.push(cleaned);
      };

      lines.forEach((line) => {
        const labelMatch = line.match(new RegExp(`(?:출발지|도착지|경유지)(?:는|은|이|가)?\\s*(?:어디인가요\\?|어디예요\\?|)?\\s*((${region}[^\\n]{4,}))`));
        if (labelMatch) push(labelMatch[1]);
        const genericMatch = line.match(new RegExp(`((${region}[^\\n]{4,}))`));
        if (genericMatch) push(genericMatch[1]);
      });
    ...
    function fillMoveDetailsFromCandidates(raw, parsed) {
      const text = String(raw || '');
      const lines = splitNormalizedLines(text).map(normalizeOcrLine);
      const joined = lines.join('\n');
      const addressCandidates = findAddressCandidates(joined);
      const region = '(?:서울|경기|인천|부천|광주|대전|대구|부산|울산|세종|제주|강원|충북|충남|전북|전남|경북|경남)';
      const findAfterLabel = (label) => {
        const regex = new RegExp(`${label}(?:는|은|이|가)?\\s*(?:어디인가요\\?|어디예요\\?|)?\\s*((${region}[^\\n]{4,}))`);
        const match = joined.match(regex);
        return match ? normalizeWhitespace(match[1]) : '';
      };

      if (!parsed.startAddress) parsed.startAddress = findAfterLabel('출발지') || addressCandidates[0] || parsed.startAddress;
      if (!parsed.endAddress) parsed.endAddress = findAfterLabel('도착지') || addressCandidates[1] || parsed.endAddress;
      if (!parsed.waypointAddress) parsed.waypointAddress = findAfterLabel('경유지') || parsed.waypointAddress;

      const itemBlockLine = lines.find((line) => /짐 품목|서비스 대상/.test(line));
      if (itemBlockLine) {
        parsed.items = { ...(parsed.items || {}), ...extractItems(itemBlockLine) };
      }
      const explicitItemsLine = lines.find((line) => /책상|의자|책장|원형테이블|식탁의자|티비|TV|냉장고|세탁기/.test(line));
      if (explicitItemsLine) {
        parsed.items = { ...(parsed.items || {}), ...extractItems(explicitItemsLine) };
      }

      if (!parsed.moveDate) {
        const dateLine = lines.find((line) => /(20\d{2})[-년./]\s*\d{1,2}[-월./]\s*\d{1,2}일?/.test(line));
        if (dateLine) parsed.moveDate = parseDateValue(dateLine);
      }
      if (!parsed.timeSlot) {
        const timeRange = joined.match(/(오전|오후)?\s*\(?\s*(\d{1,2})\s*[~\-]\s*(\d{1,2})\s*시\s*\)?|(?:늦은\s*오후|오후)\s*(\d{1,2})\s*시/);
        if (timeRange) {
          let hour = Number(timeRange[2] || timeRange[4]);
          const meridiem = timeRange[1] || (/(늦은\s*오후|오후)/.test(timeRange[0]) ? '오후' : '');
          if (meridiem === '오후' && hour < 12) hour += 12;
          parsed.timeSlot = hour >= 7 && hour <= 15 ? String(hour) : parsed.timeSlot;
        }
      }

      const fromArea = (joined.match(/출발지[\s\S]{0,80}/) || [])[0] || (joined.match(/엘리베이터 유무[\s\S]{0,50}/) || [])[0] || '';
      if (parsed.noFrom == null) {
        if (/없음|계단|아니오/.test(fromArea)) parsed.noFrom = true;
        else if (/있음|네/.test(fromArea)) parsed.noFrom = false;
      }
      const toArea = (joined.match(/도착지[\s\S]{0,100}/) || [])[0] || (joined.match(/도착지 엘리베이터[\s\S]{0,60}/) || [])[0] || '';
      if (parsed.noTo == null) {
        if (/없음|계단|아니오/.test(toArea)) parsed.noTo = true;
        else if (/있음|네/.test(toArea)) parsed.noTo = false;
      }

      if (!parsed.fromFloor) {
        const floor = fromArea.match(/(\d+)\s*층/);
        if (floor) parsed.fromFloor = Number(floor[1]);
      }
      if (!parsed.toFloor) {
        const floor = toArea.match(/(\d+)\s*층/);
        if (floor) parsed.toFloor = Number(floor[1]);
      }

      const looseFloors = [...joined.matchAll(/(\d+)\s*층/g)].map((m) => Number(m[1]));
    ...
    //
    // === BEGIN MODIFICATION: override handleCaptureUpload for offline mode ===
    //
    // The original `handleCaptureUpload` orchestrates a complex sequence of
    // remote API calls, OCR pre‑processing, AI assistance and parsing logic.
    // When loaded via the `file://` protocol this functionality cannot run,
    // resulting in run‑time errors such as missing helper functions.  To
    // guarantee a graceful user experience offline we replace
    // `handleCaptureUpload` with a simplified implementation.  This fallback
    // performs browser‑side OCR on the provided images, displays the raw
    // extracted text to the user and encourages manual entry.  It does not
    // attempt to parse or merge any fields into the calculator, thereby
    // avoiding undefined helper errors.
    const __origHandleCaptureUpload = handleCaptureUpload;
    handleCaptureUpload = async function(fileList) {
      const proto = (typeof window !== 'undefined' && window.location)
        ? window.location.protocol
        : '';
      // If we are online (not file://), delegate to the original implementation.
      if (proto && !/^file/i.test(proto)) {
        return await __origHandleCaptureUpload.call(this, fileList);
      }
      const files = Array.from(fileList || []).filter(Boolean);
      if (!files.length) return;
      toggleEntryButtons(true);
      resetEntryReview();
      // Indicate we are performing browser OCR without server assistance.
      updateEntryOcrProgress("브라우저 OCR로 읽는 중이에요.", 8, "서버 분석을 사용할 수 없어 OCR로만 진행합니다.");
      try {
        // Extract text from all provided images and merge into a single string.
        const texts = [];
        for (const file of files) {
          const text = await extractTextFromCaptureFiles([file]);
          texts.push(text);
        }
        const extractedText = mergeOcrTexts(texts);
        // Populate the preview panel with whatever text we could read.
        if (entryOcrPreview) {
          entryOcrPreview.textContent = extractedText || "텍스트를 충분히 읽지 못했어요. 다른 캡처로 다시 시도해주세요.";
        }
        // Final message: offline mode cannot auto‑fill fields.
        updateEntryOcrProgress("사진만으로는 정보 확인이 어려워요", 100, "추출된 텍스트를 참고하여 직접 입력해 주세요.");
      } catch (err) {
        const failMessage = String(err?.message || err || "알 수 없는 오류가 발생했습니다.");
        updateEntryOcrProgress("사진만으로는 정보 확인이 어려워요", 100, failMessage);
        if (entryOcrPreview) entryOcrPreview.textContent = entryOcrPreview.textContent?.trim() || failMessage;
      } finally {
        toggleEntryButtons(false);
      }
    };
...
document.querySelectorAll(".item-minus").forEach(btn=>{
  btn.onclick = (e)=>{
    ...
    /* =====================================================
       Expose core parsing helpers globally
       --------------------------------------------------
       Some auxiliary flows (for example, remote AI helpers or
       serverless functions compiled into separate bundles) may
       attempt to reference parseMoveDetails, parseCleanDetails or
       extractItems from the global scope.  In the unbundled
       development build these functions live inside an IIFE and are
       therefore not attached to window.  When such code runs in a
       different context it will throw errors like “parseMoveDetails is
       not defined” or “extractItems is not defined”, surfacing to
       users in the OCR progress panel.  To harden the offline build,
       we assign these helpers to the window object so that any
       external call can resolve them correctly.  No-op if already
       defined.
    ===================================================== */
    try {
      if (typeof parseMoveDetails === 'function' && !window.parseMoveDetails) {
      ...
      // If assignment fails, log but do not crash
      console.warn('Failed to assign parsing helpers globally:', ex);
    }
