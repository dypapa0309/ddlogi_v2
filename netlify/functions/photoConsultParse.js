...
function normalizeText(value) {
  return String(value || "")
    ...
    .replace(/\n{3,}/g, "\n\n")
    .replace(/…/g, "...")
    .replace(/\b(?:soonpfj\w*|LA\s+DERIVE|매크리스\s*이동|광고|찾으셨나요)\b/gi, " ")
    .replace(/[‐‑‒–—]/g, "-")
    .trim();
}
...
function extractAddresses(text) {
  ...
  const regex = /((?:서울|경기|인천|부천|광주|대전|대구|부산|울산|세종|제주|강원|충북|충남|전북|전남|경북|경남)(?:\s+[가-힣]+시)?\s+[가-힣0-9]+(?:시|군|구)\s+[가-힣0-9]+(?:구|군)?\s+[가-힣0-9]+(?:읍|면|동|리))/g;
  const out = [];
  let m;
...
function inferService(text) {
  const src = normalizeText(text);
  if (/(용달|화물 운송|운송|보관|이사)/.test(src)) return "move";
