  ...
  const itemsCost = items;
    ...
    function ensureAiAssistantMounted() {
      ...
      async function callAiAssistAPI({ message, targetService }) {
        // Disable calls to the remote AI assistant when running offline.
        // When this function is invoked we immediately throw to signal that
        // no server response is available.  The caller will catch this and
        // continue with locally parsed information only.  This avoids uncaught
        // promise rejections and opaque error messages when the OPENAI_API_KEY
...
${snapshotSummary}`);
    ...
    async function callCaptureVisionAPI(files) {
      ...
      // exceptions (for example, missing environment variables or undefined
      // helpers), immediately throw an error.  The surrounding logic in
      // handleCaptureUpload will catch this and fall back to client‑side OCR
      // via Tesseract.  This preserves the original flow where OCR is used as
      // a secondary source when remote analysis is unavailable.
      throw new Error("vision API disabled in offline mode");
    }

    async function callPhotoConsultAPI(files) {
      // Fall back to a purely client‑side implementation when the remote
      // photo consultation API is unreachable.  Use Tesseract.js to extract
      // text from the uploaded images and apply the existing parsing logic
      // (parseMoveDetails) to identify items.  Then construct a simple
      // summary and associated fields expected by renderPhotoConsultResult.
    ...
    //
    // In the original implementation, handleCaptureUpload coordinates a complex
    // workflow involving remote API calls, OCR pre‑processing, AI assistance and
    // detailed parsing.  When running offline via the `file://` protocol these
    // helpers are unavailable, resulting in undefined function errors.  To
    // provide a predictable offline experience we override
    // `handleCaptureUpload` with a simplified version.  This fallback performs
    // browser‑side OCR on each uploaded image, displays the extracted text in
    // the preview, and advises the user to manually enter details.  It does
    // not attempt to parse or auto‑fill form fields.
    ...
    handleCaptureUpload = async function(fileList) {
      ...
      toggleEntryButtons(true);
      resetEntryReview();
      // Indicate that we are using client‑side OCR only.
      ...
      try {
        ...
        const failMessage = String((err == null ? void 0 : err.message) || err || "알 수 없는 오류가 발생했습니다.");
        updateEntryOcrProgress("사진만으로는 정보 확인이 어려워요", 100, failMessage);
        if (entryOcrPreview) {
          entryOcrPreview.textContent = (entryOcrPreview.textContent == null ? failMessage : entryOcrPreview.textContent.trim()) || failMessage;
        }
      } finally {
        toggleEntryButtons(false);
      }
