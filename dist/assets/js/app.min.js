...
${snapshotSummary}`);
    ...
    function fillMoveDetailsFromCandidates(raw, parsed) {
      ...
      if (itemBlockLine) {
        parsed.items = { ...(parsed.items || {}), ...extractItemsSafe(itemBlockLine) };
      }
      const explicitItemsLine = lines.find((line) => /책상|의자|책장|원형테이블|식탁의자|티비|TV|냉장고|세탁기/.test(line));
      if (explicitItemsLine) {
        parsed.items = { ...(parsed.items || {}), ...extractItemsSafe(explicitItemsLine) };
      }
    ...
    photoConsultBtn?.addEventListener("click", openPhotoConsultPicker);
    photoConsultBtn?.addEventListener("touchend", (event) => {
      event.preventDefault();
      openPhotoConsultPicker();
    }, { passive: false });
