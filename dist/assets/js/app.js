...
${snapshotSummary}`);
    ...
    async function callCaptureVisionAPI(files) {
      /*
       * In offline mode (served via file://) the serverless Vision API is unreachable.
       * Under those conditions we throw to trigger OCR fallback.  When online
       * we call the Netlify function to analyse the captures, replicating
       * the original behaviour from the unmodified script.
       */
    ...
    async function callPhotoConsultAPI(files) {
      /*
       * When served online (http/https) call the Netlify function to perform
       * a photo consultation.  When offline, the existing OCR fallback
       * continues below.
       */
      {
        const proto = (typeof window !== 'undefined' && window.location)
      ...
      // Otherwise, fall back to client‑side OCR below.
    ...
    handleCaptureUpload = async function(fileList) {
      const proto = (typeof window !== 'undefined' && window.location)
        ? window.location.protocol
        : '';
      // When online (http/https) use the original implementation.
      if (proto && !/^file/i.test(proto)) {
